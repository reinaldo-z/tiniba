...
!!! BMS/FN
  ...
  !#BMSVer3.0d
  USE functions, ONLY : calVldaf,calVscissors
  !#BMSVer3.0u
  ...
  ! debug
  if(vnlkss)then
     write(*,*)'set_input_ascii.f90: vnl will be included'
  else
     write(*,*)'set_input_ascii.f90: vnl not included'
  end if
...
!#BMSVer3.0d
           IF ( layeredCalculation ) then
              !contains v^\nl if option -n is chosen
              !Does NOT contain scissors, as it must.
              calVlda(:,iv,ic)=calMomMatElem(:,iv,ic)
              IF (ic.NE.iv) THEN
              calVlda(:,ic,iv)=conjg(calVlda(:,iv,ic))
              end IF
           end IF
!#BMSVer3.0u
!#BMSVer3.0d
           ...
           do iv=ic,nMax
              do ii=1,3
                 ...
                 do l=1,nMax
                    ...
                    if(l.ne.iv) then
                       t2=-cfMatElem(ic,l)*posMatElem(ii,l,iv)
                       t1=t1+t2
                    end if
                 end do
                 if ( ic .ne. iv ) then
...
!!$     end IF
     !#BMSVer3.0u
     !#BMSVer3.0d
     !Generates {\cal V}^\lda_{nm} so they are available and is faster
!!$     IF ( layeredCalculation ) then
!!$        do ic=1,nMax
!!$           do iv=ic,nMax
!!$              do ii=1,3
!!$                 !below is valid only for ala-bms
!!$                 !and doesn't work, dunno why!
...
!!$     end IF
     ...
     !assumes that calv^\lda is calculated directly from ???
     ! (n.ne.m)
     IF ( layeredCalculation ) then
        do ic=1,nMax
           do iv=ic+1,nMax
...
!!$        end if
     ...
     IF ( layeredCalculation ) then
        ...
        if ( ik .eq. 1 ) then 
           write(*,*)'********'
           if(layeredInjectionCurrent) then
              write(*,*)'set_input_ascii.f90: layered injection current'
           else
     ...
     !so change 2->1 if needed
     if (1.eq.1)then
        IF ( layeredInjectionCurrent ) THEN
           do iv=1,nMax
              do ii=1,3
                 if(vnlkss)then
                    if((ik.eq.1).and.(iv.eq.1).and.(ii.eq.1))write(*,*)"set_input_ascii.f90: layeredInjectionCurrent with vnl"
                    if ( scissor .gt. 0.d0 ) then
                       calVsig(ii,iv,iv)=calVlda(ii,iv,iv)&
                            +calVscissors(ii,iv,iv,ik)*scissor
                    else
                       calVsig(ii,iv,iv)=calVlda(ii,iv,iv)
                    end if
                 else
                    if((ik.eq.1).and.(iv.eq.1).and.(ii.eq.1))write(*,*)"set_input_ascii.f90: layeredInjectionCurrent without vnl"
                    calVsig(ii,iv,iv)=curMatElem(ii,iv)
                 end IF
              end do
           end do
        end if
