...
printf "   -c      Layered Momentum calp_{mn}(k) and Cut Function calf_{mn}(k) Matrix Elements\n"
...
#
# selects the name of the architecture without the numerical postfix
nodin=`head -1 .machines_scf.original |grep -oE "[[:lower:]]{1,}"`
#
if [ "$nodin" == "quad" ]
then
...
then
    mv .machines_scf.original .machines_scf.hexa
    Line
...
## checks that the options are given
    if [[ $wf = 'false' ]] && [[ $rho = 'false' ]] && [[ $em = 'false' ]] && [[ $pmn = 'false' ]] && [[ $rhoccp = 'false' ]] && [[ $lpmn = 'false' ]] && [[ $lpmm = 'false' ]] && [[ $sccp = 'false' ]] && [[ $lsccp = 'false' ]]   
    then
	Line
...
## saves info of the run parameters
    echo "#!/bin/bash"> como_corri_run_tiniba.sh
    echo "`date`      ">> como_corri_run_tiniba.sh
    echo "`dirname $0`/`basename $0` $@ " >> como_corri_run_tiniba.sh
    chmod 777 como_corri_run_tiniba.sh
...
##
    if [[ $layers == 0 ]] && [[ $rhoccp == 'true' ]] && [[ $lpmn == 'true' || $lpmm == 'true' || $lsccp == 'true' ]]
	then
	Line
...
fecha=`date`
echo $fecha >> .info_run
echo "$0 -r $action -k $Nk -N $layers -x $serialp wf(-w)=$wf rho(m)=$rho em(-e)=$em pmn(-p)=$pmn rhomn(-d)=$rhoccp lpmn(-c)=$lpmn lpmm(-l)=$lpmm sccp(-s)=$sccp lsccp(-n)=$lsccp wfcheck(-b)=$wfcheck" >> .info_run
# this mimics a goto
#    goto='yes'
    goto='no'
if [ $goto == 'no' ]
...
# run all-nodes.sh
    $where/$cual 1 $Nk $Nlayer $serialp $last_name $wf $rho $em $pmn $rhoccp $lpmn $lpmm $sccp $lsccp $wfcheck
#
# if -w is the only option, the shell is done and exits
#
    if [[ $wf == 'true' ]] && [[ $rho == 'false' ]] && [[ $em == 'false' ]] && [[ $pmn == 'false' ]] && [[ $rhoccp == 'false' ]] && [[ $lpmn == 'false' ]] && [[ $lpmm == 'false' ]] && [[ $sccp == 'false' ]] && [[ $lsccp == 'false' ]]
    then
	Line
...
then
    if [ $layers == 'half-slab' ]
    then
#	mv rhoccpd_$Nkl'_1' me_rhoccp_$Nkl'_half-slab'$last_name 
	mv rhoccpd_$Nkl'_1' me_rhoccp_$Nkl'_half-slab'$last_name 
    else
	...
	if [ "$whichlayers" == "all" ]
	    ...
	    while [ $nn -lt $Nlayer ]
	      do
	      nn=`expr $nn + 1`
#	      mv rhoccpd_$Nkl'_'$nn  me_rhoccp_$Nkl'_'$nn$last_name
	      mv rhoccpd_$Nkl'_'$nn  me_rhoccp_$Nkl'_'$nn$last_name
	    done
	else
	    nn=0
	    for lname in ${whichlayers[@]}
	      do
	      nn=`expr $nn + 1`
#	      mv rhoccpd_$Nkl'_'$nn  me_rhoccp_$Nkl'_'$lname$last_name
	      mv rhoccpd_$Nkl'_'$nn  me_rhoccp_$Nkl'_'$lname$last_name
	    done
	fi
    fi
