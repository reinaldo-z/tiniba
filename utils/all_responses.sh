...
#BMSVer3.0d
	...
	if [ "$lt" == "layer" ] 
	then
	    printf "\tmove me_cpmn_$caso to me_cpmn_$caso-o\n"
	    mv me_cpmn_$caso me_cpmn_$caso-o
	    printf "\tcp me_calvnlnm_$caso to me_cpmn_$caso\n"
	    cp me_calvnlnm_$caso me_cpmn_$caso
	fi
	Line
...
### return me_pmn*-o to me_pmn*
	...
	then
	    ...
	    cp me_pmn_$caso-o me_pmn_$caso
	    fi
	    if [ "$lt" == "layer" ] 
	    then
		#take out the layer info from $caso
		...
		printf "\tme_cpmn_$caso-o is moved to me_cpmn_$caso\n"
		cp me_pmn_$casovnl-o me_pmn_$casovnl
		cp me_cpmn_$caso-o  me_cpmn_$caso
	    fi
